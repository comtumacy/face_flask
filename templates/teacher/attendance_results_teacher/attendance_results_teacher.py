...
import json
from teacher.attendance_results_teacher.attendance_results_sql import attendance_results_sql
...
def attendance_results_teacher_fun():
    ...
    if token != token_get:
        ...
        all_list, status = attendance_results_sql(get_data['class'] + get_data['classno'], get_data['date'])
        if status != 0:
            post_data = all_list
            #  返回的内容
            response = make_response(json.dumps(post_data))
